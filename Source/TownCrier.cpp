	...
	if (CWeenieObject *pOther = g_pWorld->FindObject(m_LastUsedBy))
	{
		...
		if (g_pConfig->TownCrierBuffs())
		{
			...
			case 7:
				...
				//Seedsow Files
				m_SpellcastingManager->CastSpellInstant(pOther->GetID(), RejuvenationOther7_SpellID);
				m_SpellcastingManager->CastSpellInstant(pOther->GetID(), RegenerationOther7_SpellID);
				m_SpellcastingManager->CastSpellInstant(pOther->GetID(), ManaRenewalOther7_SpellID);
				m_SpellcastingManager->CastSpellInstant(pOther->GetID(), ArcaneEnlightenmentOther7_SpellID);
				m_SpellcastingManager->CastSpellInstant(pOther->GetID(), LeadershipMasteryOther7_SpellID);
